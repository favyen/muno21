...
func visualize(clusters []lib.Cluster, jpgDir string, graphDir string, outDir string, annotations []lib.Annotation) {
	...
	for i, cluster := range clusters {
		...
		drawCluster := func(im [][][3]uint8, offset [2]int, oldIdx int, newIdx int) {
			...
			// draw the old graph in blue, slightly offset
			...
			for _, edge := range graphs[oldIdx].WithoutAll.Search(rect) {
				...
				drawSegment(im, offset, segment, [3]uint8{0, 0, 255}, 0)
			}

			// draw the new graph in yellow
			for _, edge := range graphs[newIdx].WithAll.Search(rect) {
				drawSegment(im, offset, edge.Segment(), [3]uint8{255, 127, 0}, 0)
			}
			for _, edge := range graphs[newIdx].WithoutAll.Search(rect) {
				drawSegment(im, offset, edge.Segment(), [3]uint8{255, 255, 0}, 0)
			}
		}

		// for individual image
		drawChange := func(im [][][3]uint8, offset [2]int, change lib.ChangedRoad) {
			var color [3]uint8
			if change.Deleted {
				color = [3]uint8{255, 0, 0}
			} else {
			...
			for _, segment := range change.Segments {
				drawSegment(im, offset, segment, color, 0)
			}
		}
		...
		// write per-change images
		if false {
		for j, change := range cluster.Changes {
			//rect := change.GetRectangle()
			//changeRect := [4]int{int(rect.Min.X), int(rect.Min.Y), int(rect.Max.X), int(rect.Max.Y)}
			//changeRect = clipPad(changeRect, SmallPadding, dims)
			changeRect := summaryRect
			for _, im := range ims {
				changeIm := image.Crop(im.Image, changeRect[0], changeRect[1], changeRect[2], changeRect[3])
				drawChange(changeIm, [2]int{changeRect[0], changeRect[1]}, change)
				image.WriteImage(filepath.Join(clusterDir, fmt.Sprintf("%d_%s.jpg", j, im.Timestamp)), changeIm)
			}
		}
		}

		// write meta information
		s := fmt.Sprintf("%s\n%s\n%v\n", cluster.FirstTimestamp, cluster.LastTimestamp, summaryRect)
		if err := ioutil.WriteFile(filepath.Join(clusterDir, "meta.txt"), []byte(s), 0644); err != nil {
			panic(err)
		}
	}
...
<table>
	...
	for i, cluster := range clusters {
		...
		`, i, i, firstTimestamp, i, lastTimestamp, region, years, tags)
	}
	html += `
	</tbody>
</table>
