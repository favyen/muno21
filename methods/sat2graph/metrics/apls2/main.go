...
var region_size_x float64 = 2048.0 // unit is meter
var region_size_y float64 = 2048.0 // unit is meter
var margin_size float64 = 64.0 // unit is meter
...
func GPSInBound(p1 [2]float64) bool {
	...
	lat2 := lat_top_left - region_size_y/111111.0
	lon2 := lon_top_left + region_size_x/111111.0 / math.Cos(lat_top_left/180.0 * 3.1415926)
...
func apls_one_way(graph_gt *graph, graph_prop *graph, graph_extra *graph, ret chan float64) {
	...
	if len(control_point_gt) > 200 {
		new_map := make(map[int]int)
		var keys []int
		for k := range control_point_gt {
			keys = append(keys, k)
		}
		for _, idx := range rand.Perm(len(keys))[0:200] {
			k := keys[idx]
			new_map[k] = control_point_gt[k]
		}
		fmt.Printf("reduce ground truth map control points from %d to %d\n", len(control_point_gt), len(new_map))
		control_point_gt = new_map
	}
	...
	// do the same for _extra graph
	rt = rtreego.NewTree(2, 25, 50)
	node_cover_map = make(map[int]bool)
	control_point_extra := make(map[int]int)
	for nid1 := range control_point_gt {
		control_point_extra[nid1] = -1
	}
	for nid, loc := range graph_extra.Nodes {
		rt.Insert(&gpsnode{
			nid: nid,
			loc: loc,
		})
	...
	for nid1, _ := range control_point_extra {
		q := rtreego.Point{graph_gt.Nodes[nid1][0], graph_gt.Nodes[nid1][1]}
		results := rt.NearestNeighbors(100, q)
		for _, result := range results {
			if node_cover_map[result.(*gpsnode).nid] == true {
				continue
			}
			if GPSDistance(result.(*gpsnode).loc, graph_gt.Nodes[nid1]) < 10.0 {
				control_point_extra[nid1] = result.(*gpsnode).nid
				graph_extra.propagate(result.(*gpsnode).nid, prop_step, func(nid int){
						node_cover_map[nid] = true
					})
				break
			}
		}
		if control_point_extra[nid1] == -1 {
	...
	var control_point_extra_list []int
	...
	for cp1_gt, cp1_prop := range  control_point_gt {
		...
		control_point_extra_list = append(control_point_extra_list, control_point_extra[cp1_gt])
	...
	shortest_paths_extra := make(map[int]map[int]float64)
	...
	counter = 0
	for _, cp_extra := range  control_point_extra_list {
		shortest_paths_extra[cp_extra], _ = graph_extra.ShortestPaths(cp_extra, control_point_extra_list )
		counter += 1
		if counter % 100 == 0 {
			fmt.Println("computing extra graph shortest paths ", counter, len(control_point_extra_list))
		}
	}
	...
	for cp1_gt, cp1_prop := range  control_point_gt {
		for cp2_gt, cp2_prop := range  control_point_gt {
			...
			cp1_extra := control_point_extra[cp1_gt]
			cp2_extra := control_point_extra[cp2_gt]
			...
			d3 := shortest_paths_prop[cp1_extra][cp2_extra]
			fmt.Println("OK OK", d1, d3, shortest_paths_prop[cp1_prop][cp2_prop])
			if d1 > min_distance_filter && d3 > min_distance_filter {
...
func apls(graph_gt *graph, graph_prop *graph, graph_extra *graph) {
	c1 := make(chan float64, 1)
	c2 := make(chan float64, 1)

	go apls_one_way(graph_gt, graph_prop, graph_extra, c1)
	go apls_one_way(graph_prop, graph_gt, graph_extra, c2)
...
func main() {
	...
	graph_extra := LoadGraphFromJson(os.Args[3])
	...
	region_size_x, _ = strconv.ParseFloat(os.Args[3], 64)
	region_size_y, _ = strconv.ParseFloat(os.Args[4], 64)


	graph_gt_dense := GraphDensify(graph_gt)
	graph_prop_dense := GraphDensify(graph_prop)
	graph_extra_dense := GraphDensify(graph_extra)

	apls(graph_gt_dense, graph_prop_dense, graph_extra_dense)
