...
class Sat2GraphDataLoader():
	...
	def loadtile(self, ind):
		...
		try:
			sat_img = skimage.io.imread(self.folder + "/region_%d_sat.png" % ind).astype(np.float)
		except:
			sat_img = skimage.io.imread(self.folder + "/region_%d_sat.jpg" % ind).astype(np.float)
	...
	def preload(self, num = 1024, seg_only = False):
		...
		for i in range(self.preload_tiles):
			...
			try:
				sat_img = skimage.io.imread(self.folder + "/region_%d_sat.png" % ind)
			except:
				sat_img = skimage.io.imread(self.folder + "/region_%d_sat.jpg" % ind)
			...
			gt_seg = skimage.io.imread(self.folder + "/region_%d_gt.png" % ind)
